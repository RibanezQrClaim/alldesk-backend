...
from llm_client import analizar_mensaje_llm  # al inicio de app.py
...
# --- PROCESAR TICKET ENTRANTE ---

def procesar_ticket_entrante(remitente, asunto, cuerpo, canal):
    analisis = analizar_mensaje_llm(f"{asunto}\n{cuerpo}")
    tipo = analisis.get("tipo", "Consulta")
    prioridad = analisis.get("prioridad", "Normal")  # Si decides agregarlo a la base
    clasificacion = analisis.get("clasificacion", "Otro")  # Igual aquí si usas este dato
...
def recibir_email_webhook():
    ...
    procesar_ticket_entrante(remitente=sender, asunto=subject, cuerpo=body, canal="Email")
